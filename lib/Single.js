...
Single.prototype._flush = function (cb) {
  ...
  this._connection.request(calls, function(err, responseArray){
    var response, err, results, stats;
    try {
    ...
    callbacks.forEach(function(itemCb,i){
      if(itemCb) itemCb(err,results[i],stats[i]);
    })
...
Single.prototype.query = function ( query, p , c ) {
  ...
  this._pending.push([call, function(err,results,info){
...
function cbMulti (cb) {
  return function (err, results ) {
    if (cb) cb(err, results && results.map(function(item){return item[0]}) );
  }
} 

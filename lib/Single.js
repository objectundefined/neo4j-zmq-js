...
Single.prototype.getNode = function ( node, labels, properties, cb ) {
  var call = compose("GET", "node", [ extractNode(node), labels, properties]);
...
Single.prototype.getNodeByProperty = function ( label, key, value, cb ) {
  var call = compose("GET", "nodeset", [label, key, value]);
  this._pending.push([call,cbSingle(cb)]);
  return this._flush();  
}

Single.prototype.mergeNodeByProperty = function ( label, key, value, cb ) {
  var call = compose("PUT", "nodeset", [label, key, value]);
  this._pending.push([call,cbSingle(cb)]);
  return this._flush();  
}

Single.prototype.purgeNodesByProperty = function ( label, key, value, cb ) {
